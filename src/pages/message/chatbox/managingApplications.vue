<template>
  ...
  <view class="chatbox" v-if="!isLoad">
    <u-navbar title="管理申请" autoBack placeholder bgColor="#f5f5f5" titleStyle="font-weight:bold">
    </u-navbar>
    ...
    <view class="chatbox__search-list-item">
      ...
      >
        ...
        <text v-if="latestMessageList.length === 0 && frontMessageList.length === 0"
          >暂无管理申请</text
        >
        <view v-for="item in latestMessageList" :key="item.item.id">
          <item :item="item" :isShare="false" :userId="userId" />
        </view>
        <view class="chatbox__search-list-item__time" v-if="frontMessageList.length !== 0">
          <u-text text="三天前"></u-text>
        </view>
        <view v-for="item in frontMessageList" :key="item.item.id">
          <item :item="item" :isShare="false" :userId="userId" />
        </view>
      </scroll-view>
    </view>
...
onLoad(async () => {
  ...
  // 获取最新信息
  await messageStore.fetchNewMessageList(1)
  // 隐藏骨架屏
  isLoad.value = false
})
...
// 时间判断(将信息区分为三天前、近三天)
watch(
  () => currentMessageList.value.messageList.map((item) => item.id),
  () => {
